...
def main(target_bus, target_station, img_path):
    ...
    if diff_color > 0:
        detected_color = detect_color(
            img_path, leftup=leftup, rightdown=rightdown)
        if detected_color != target_color:
            return 'Unexpected Color'

    if same_color > 1 and detect_number(img_path, target_bus, leftup, rightdown) == False:
    ...
    # Calculation
    door_leftup = np.array((door['x'], door['y']))
    door_rightdown = np.array((door['x'] + door['w'], door['y'] + door['h']))
    distance, angle = calculate_distance_angle(door_leftup, door_rightdown, radian)

    return str(round(distance/1000, 2)) + " meter", str(round(angle*180/np.pi)) + " 도"

print('RESULT:', main(target_bus='4211', target_station='23322', img_path='./input/bus4211.jpg'))
